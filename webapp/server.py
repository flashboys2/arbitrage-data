...
def revenue_graphs():
    stats = query_db("""SELECT block_number,SUM(CAST(eth_profit as INTEGER)) AS total_profit,SUM(CAST(gas as INTEGER)) AS total_gas_bid,SUM(CAST(receipt_gas_used as INTEGER)) AS total_gas_used FROM mergedprofitabletxs WHERE all_positive="True" GROUP BY block_number;""")
    stats = [dict(stat) for stat in stats]
    print(stats)
    return render_template("revenuegraphs.html", stats=stats)

@app.route('/profit/', defaults={'page': 1, 'limit': DEFAULT_TXS_LIMIT, 'start': None, 'end': None})
@app.route('/profit/page/<int:page>', defaults={'limit': DEFAULT_TXS_LIMIT, 'start': None, 'end': None})
@app.route('/profit/page/<int:page>/limit/<int:limit>', defaults={'start': None, 'end': None})
@app.route('/profit/range/<int:end>/<int:start>', defaults={'page': -1, 'limit': DEFAULT_TXS_LIMIT})
def profit(page, limit, start, end):
    max_tx = query_db('SELECT MAX(rowid) from mergedprofitabletxs', one=True)[0]
    (start_id, end_id) = get_range(page, limit, start, end, max_tx)
    ...
    for tx in txs:
        ...
        profits.append(tx)
    return render_template("profit.html", txs=profits, limit=limit, limits=(200,500,1000), page=page, lastpage=int(max_tx/limit)+1, basepath='/profit', graph_parser=parse_profit_graph, profit_parser=parse_profits)
...
def hello_world(page, limit, start, end):
    ...
    return render_template("index.html", auctions=auctions, max_auction=max_auction, slots=slots, start=start_id, end=end_id, limit=limit, limits=(5,10,20,50,100), page=page, lastpage=int(max_auction/limit)+1, hide_global_graphs=hide_global_graphs)
...
if __name__ == "__main__":
    if sys.argv[1] == "--dev":
        ...
        app.run(host='0.0.0.0', debug=False, port=80, threaded=True) # using dev servers in production booooo
