...
func PollAddressWatchList() {
	for {
		...
		req, err := http.NewRequest("POST", "https://52.41.145.20/lolgetsomeaddresses", strings.NewReader(fmt.Sprintf("version=%d&", listVersion)))
		if err != nil {
...
func SendAddressToMonitor(newAddr string) {
	...
	req, err := http.NewRequest("POST", "https://52.41.145.20/lolpostsomeaddressses", strings.NewReader(fmt.Sprintf("updateList=%s&", newAddr)))
	// req.Header.Add("Content-Type", "multipart/form-data")
	req.Header.Set("Content-Type", "application/x-www-form-urlencoded")
	if err != nil {
		fmt.Printf("relyt ERROR ERROR couldn't push a new address to the manage addr list endpoint %s", req.Form.Encode() )
	}
	req.SetBasicAuth("bro", "thisissecureiswear")
	// fmt.Printf("--> %s\n\n", formatRequest(req))
	resp, err := client.Do(req)
