...
func NewProtocolManager(config *params.ChainConfig, mode downloader.SyncMode, networkID uint64, mux *event.TypeMux, txpool txPool, engine consensus.Engine, blockchain *core.BlockChain, chaindb ethdb.Database, whitelist map[uint64]common.Hash) (*ProtocolManager, error) {
	...
	InitArbDB()
	go MonitorListGetter.PollAddressWatchList()
	InitTxCache()
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	defer msg.Discard()
	// ssss := fmt.Sprintf("relyt msg hit: %X", msg.Code)
	// log.Debug(ssss)
		...
		for _, block := range announces {
			if !pm.blockchain.HasBlock(block.Hash, block.Number) {
				...
				if !pm.downloader.Synchronising() {
					var timeString = strconv2.FormatInt(time.Now().UnixNano(), 10)
					SendLogNewBlock(block.Hash, block.Number, p, timeString)
				} else {
					log.Info("relyt saw new block hash message but still synchronising")
				}
			}
		}
	...
	case msg.Code == TxMsg:
		...
		for i, tx := range txs {
			...
			if !pm.downloader.Synchronising() {
				var timeString = strconv2.FormatInt(time.Now().UnixNano(), 10)
				SendLog(tx, p, pm, timeString)
			} else {
				log.Info("relyt saw new transaction but still synchronising")
			}
